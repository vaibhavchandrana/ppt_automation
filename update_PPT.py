    ...
    for key, value in third_slide_data.items():
        if isinstance(value, (int, float)):
            ...
            if value > 100 and key != "Average_time_spent":
                validation = False
                print(key, " has value ", value,    " which can not be greater than 100 ")
            ...
            if key == "Gender_Split_inside_store":
                ...
                validation = False
                print(key, " has value  ", value, " which is not valid ( it should be digit )")
    return validation
...
def validateAndCalculateFourthSlideData(fourth_chart_data,passerby_vs_footfall):
    ...
    # checking if there is no passerby then there should be no footfall
        if fourth_chart_data['passerby'][i] == 0:
            if fourth_chart_data['footfall'][i] == 0:
            ...
            per=round((fourth_chart_data['footfall'][i]/fourth_chart_data['passerby'][i])*100,2)
            if per < 0.5:
                chart_data_validation = False
                print("passerby value and footfall value is not correct")
                break
            ...
            if fourth_chart_data['footfall'][i] > fourth_chart_data['passerby'][i]:
                chart_data_validation = False
